...
def parse_args():
    ...
    parser.add_argument(
        "--band", type=str, default="i", required=False, help="band name"
    )
...
def process_patch(entry, skymap, task, band):
    ...
    out_fname = os.path.join(out_dir, "fpfs_%s.fits" % (band))
...
def main():
    ...
    config.fpfs.do_compute_detect_weight = False
    config.fpfs.psf_model_type = "object"
    config.fpfs.npix = 64
    config.fpfs.sigma_shapelets1 = 0.52
    task = FpfsForcePipe(config=config)

    for entry in my_entries:
        process_patch(entry, skymap, task, args.band)
        gc.collect()
    return
