#!/usr/bin/env python3
"""Merge per-tract extinction files into per-field files.

For each field in FIELDS, reads the list of tracts from
fields_color/{field}.fits, concatenates the corresponding
tracts_extinction/{tract}.fits files, sorts by object_id, and writes
fields_extinction/{field}.fits.
...
TRACT_DIR = f"{BASE}/tracts_extinction"
COLOR_DIR = f"{BASE}/fields_color"
OUT_DIR = f"{BASE}/fields_extinction"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Merge per-tract extinction files into per-field files."
    )
    parser.add_argument(
