#!/usr/bin/env python3
"""Global stats accumulated over all 6 fields.

Loops over each field, reads fields_multiband + fields_redshift,
accumulates weighted sums, then computes the combined statistics."""
...
# Accumulators
N_cut = 0         # objects after cut
sum_we1 = 0.0     # sum(w*e1) over cut
sum_we2 = 0.0
sum_we1sq = 0.0   # sum((w*e1)^2) over cut
sum_we2sq = 0.0
sum_R11 = 0.0     # sum(w*de1/dg1 + dw1*e1) over cut
sum_R22 = 0.0
sum_w = 0.0
# Selection response numerators (over ALL objects, not just cut)
sum_wp1 = 0.0     # sum(w[cut_p1]*e1[cut_p1])
sum_wm1 = 0.0
sum_wp2 = 0.0
sum_wm2 = 0.0
...
for field in FIELDS:
    ...
    w = d["wsel"][cut].astype(np.float64)
    ...
    sum_R11 += np.sum(w * de11 + dw1 * e1c)
    sum_R22 += np.sum(w * de22 + dw2 * e2c)
    sum_w += np.sum(w)

    # Selection response sums (full sample)
    w_all = d["wsel"].astype(np.float64)
    ...
    sum_wp1 += np.sum(w_all[cut_p1] * e1_all[cut_p1])
    sum_wm1 += np.sum(w_all[cut_m1] * e1_all[cut_m1])
    sum_wp2 += np.sum(w_all[cut_p2] * e2_all[cut_p2])
    sum_wm2 += np.sum(w_all[cut_m2] * e2_all[cut_m2])
...
std_avg = np.sqrt(0.5 * (var_we1 + var_we2))

print(f"\n=== Global stats (all fields, zkey={zkey}) ===")
...
print(f"{'R11 (shear response)':<30} {R11:>18.6f}")
print(f"{'R22 (shear response)':<30} {R22:>18.6f}")
...
print(f"{'std_avg / R_avg':<30} {std_avg / R_avg:>18.6f}")
