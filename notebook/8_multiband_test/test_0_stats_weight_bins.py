...
accumulated over all 6 fields, in 5 redshift bins.

Bins: (0.3,0.6], (0.6,0.9], (0.9,1.2], (1.2,1.5], (1.5,1.8].
All accumulators are length-5 arrays (one element per z-bin).
...
# Weight bias: sum(dw*wopt*e + wsel*dwopt*e) per component
...
for field in FIELDS:
    ...
    common_kwargs_noz = dict(
        zbin=None, ext=ext, emax=args.emax, mag_cuts=mag_cuts,
    )
    base_cut = get_cut(d, comp=1, dg_eff=0.0, **common_kwargs_noz)

    # Perturbed cuts for selection response (no z-cut in get_cut;
    # z-bin assignment uses the perturbed z below)
    ...
    # Pre-compute per-object arrays (before z-bin loop)
    ...
    for ib, (z_lo, z_hi) in enumerate(Z_BINS):
        ...
        # Weight bias: the non-de/dg terms in the response
        sum_wb1[ib] += np.sum(dw1 * wopt_c * e1c + wsel_c * dwopt_dg1 * e1c)
        sum_wb2[ib] += np.sum(dw2 * wopt_c * e2c + wsel_c * dwopt_dg2 * e2c)
        ...
        sum_wp1[ib] += np.sum(w_total[cp1] * e1_all[cp1])
        sum_wm1[ib] += np.sum(w_total[cm1] * e1_all[cm1])
        sum_wp2[ib] += np.sum(w_total[cp2] * e2_all[cp2])
        sum_wm2[ib] += np.sum(w_total[cm2] * e2_all[cm2])
...
print(
    f"\n=== Weighted stats per z-bin (all fields, "
...
# ---- Write per-bin selection bias and weight bias to FITS ----
sel_bias = np.zeros(NBINS)
weight_bias = np.zeros(NBINS)
for ib in range(NBINS):
    ...
    r_sel_1 = (sum_wp1[ib] - sum_wm1[ib]) / (2.0 * dg) / nc
    r_sel_2 = (sum_wp2[ib] - sum_wm2[ib]) / (2.0 * dg) / nc
    sel_bias[ib] = 0.5 * (r_sel_1 + r_sel_2)
    weight_bias[ib] = 0.5 * (sum_wb1[ib] / nc + sum_wb2[ib] / nc)
...
out_arr = np.zeros(
    ...
    dtype=[
        ("z_lo", "f8"), ("z_hi", "f8"),
        ("N_cut", "i8"),
        ("sel_bias", "f8"), ("weight_bias", "f8"),
    ],
)
for ib, (z_lo, z_hi) in enumerate(Z_BINS):
    out_arr["z_lo"][ib] = z_lo
    out_arr["z_hi"][ib] = z_hi
    out_arr["N_cut"][ib] = N_cut[ib]
    out_arr["sel_bias"][ib] = sel_bias[ib]
    out_arr["weight_bias"][ib] = weight_bias[ib]

outdir = f"{BASE}"
outfname = f"{outdir}/bias_per_zbin_imag{args.imag:.1f}.fits"
fitsio.write(outfname, out_arr, clobber=True)
print(f"\nWrote per-bin biases to {outfname}")
