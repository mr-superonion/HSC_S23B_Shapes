...
def get_shape(iband, mb):
    e1 = mb["e1"] * iband["wsel"]
    e2 = mb["e2"] * iband["wsel"]
    r1 = (
        mb["de1_dg1"] * iband["wsel"] +
        iband["dwsel_dg1"] * mb["e1"]
    )
    r2 = (
        mb["de2_dg2"] * iband["wsel"] +
        iband["dwsel_dg2"] * mb["e2"]
    )
    return {
        "e1": e1,
        "e2": e2,
...
def process_tract(tract_id, emax_sq, mag_cuts):
    ...
    fname3 = f"{os.environ['s23b_anacal_v2']}/tracts_color/{tract_id}.fits"
    color = fitsio.read(fname3)

    iband = iband[mask]
    mb = mb[mask]
    color = color[mask]

    shape = get_shape(iband, mb)
    ...
    gr = test_color_bin(iband, shape, "gr")
    ri = test_color_bin(iband, shape, "ri")
    iz = test_color_bin(iband, shape, "iz")
    zy = test_color_bin(iband, shape, "zy")
    return {
        "psf2": psf2,
        "psf4": psf4,
        "var": var,
...
def main():
    ...
    for tract_id in my_entries:
        out = process_tract(tract_id, args.emax ** 2, mag_cuts_local, mag_cuts_local)
    ...
    if rank == 0:
        outdir = f"{os.environ['s23b_anacal_v2']}/tests_multiband"
        os.makedirs(outdir, exist_ok=True)
        for tt in test_names:
            outfname = f"{outdir}/{tt}_stack.fits"
