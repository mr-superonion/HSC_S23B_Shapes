#!/bin/bash
# Usage: ./submit_fields_mini.sh <script_name.py> [extra_args...]
# Example: ./submit_fields_mini.sh make_1_bin_selection.py --imag 25.8
...
cd "$PWD"
mpirun -np 20 python $SCRIPT --field "$field" $EXTRA_ARGS
EOF
    sleep 10
done
